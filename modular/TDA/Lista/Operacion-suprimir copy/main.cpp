...
enum {ALTA_ELEMENTO=1,BAJA_ELEMENTO,LISTADO_ELEMENTOS,BUSCA_ELEMENTO,SALIR};
...
void buscarIngreso(){
    ...
    cout << "Busqueda de un ingreso" << endl << endl  
         << "Dime que valor de ingreso buscar: ";
    cin >> ingreso;
    p = localiza(ingreso, ingresos);
    if( p!=fin(ingresos) ){
        cout << "El ingreso si se encuentra" << endl;
    } else {
        cout << "El ingreso no se encuentra en la lista de ingresos" << endl;
    }
}