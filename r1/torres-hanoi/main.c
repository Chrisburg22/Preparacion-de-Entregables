...
int hanoi(int numDiscos, int torreInicial, int torreAuxiliar, int torreFinal)
{
	...
	static const int arrTorreInicial[numDiscos];
	static const int arrTorreAuxiliar[numDiscos];
	static const int arrTorrreFinal[numDiscos];
	if(numDiscos == 1) // solo hay un disco
	{
		printf("Mover el disco superior de la torre %d a la Porre %d\n", torreInicial, torreFinal);
		arrTorrreFinal[numDiscos - 1] = arrTorreInicial[numDiscos-1];
		arrTorreInicial[numDiscos-1]=0;
		totalMovimientos++;
	}
